...
class FoodDetailViewController : UIViewController
{
    ...
    @IBOutlet weak var foodDecriptionLabel: UILabel!
    ...
    override func viewDidLoad() {
        ...
        foodDecriptionLabel.text = food.description
    ...
    @IBAction func addToCart(_ sender: Any)
    {
        ...
        imageView.image = self.foodImageView.image
        ...
        UIView.animate(withDuration: 1.0, delay: 0.0, options: .curveEaseOut, animations: {
            ...
            let cartItem = CartItem(food: self.food, quantity: self.quantity)
            ...
            //Else, if ordered foods same restaurant. Existing cart
            if cartRestaurant.id == currentRestaurant.id {
                ...
                let itemIndex = Cart.currentCart.items.index(where: { (currentItem) -> Bool in
                    return currentItem.food.id == cartItem.food.id
                })
                
                if let itemIndex = itemIndex {
                    // Already added in cary
                    //Add alert want to add more same food
                    ...
                    // User want to add foods don't exist in cart
                    Cart.currentCart.items.append(cartItem)
                    
                    
                }
            } else {
                //Foods not from same restaurant
